...
fn display_error(err: Error) -> TokenStream {
    match err {
        ...
        Error::PeekOnEmptyNode => panic!("Internal error; peeked on an empty node (this should never be visible)"),
        Error::ChompOnEmptyNode => panic!("Internal error; chomped on an empty node (this should never be visible)"),
        Error::FmtError(fmt) => panic!("Internal error; failed writing to string (this should never be visible), {}", fmt),
    }
}